...
IDA分析后函数的参数大概这样，而且是个stdcall，比较好弄，先把特征码弄过来。这里说明一下，如果是fastcall,64位估计很难调用，__fastcall采用寄存器传递参数前两个参数，32位好像是ecx和edx，64位的是rax和rcx, 64位c++又不能内联汇编，所以很难，有些IDA反汇编后显示__usercall大多数也是fastcall。    
